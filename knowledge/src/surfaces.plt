...
test(is_furniture) :-
    triple(Furniture, urdf:'hasURDFName', 'couch_table#table#table_front_edge_center'),
...
test(is_surface) :-
    triple(Surface, urdf:'hasURDFName', 'couch_table#table#table_center'),
    is_surface(Surface).

test(all_surfaces) :-
    all_surfaces(Surfaces),
    length(Surfaces, Count),
    assert_equals(Count, 14).

test(furniture_surfaces) :-
    triple(Furniture, urdf:'hasURDFName', 'bookshelf#shelf#shelf_base_center'),
    furniture_surfaces(Furniture, Surfaces),
    length(Surfaces, Count),
    assert_equals(Count, 3).

test(has_table_shape) :-
    triple(Furniture, urdf:'hasURDFName', 'couch_table#table#table_front_edge_center'),
    has_table_shape(Furniture).

test(has_shelf_shape) :-
    triple(Furniture, urdf:'hasURDFName', 'bookshelf#shelf#shelf_base_center'),
